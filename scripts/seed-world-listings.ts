...
// GeoNames city data (generated by scripts/prepare-geonames.ts).
// If not yet generated, falls back to EXTENDED_CITIES (~1 200 cities).
let GEONAMES_CITIES: Array<[string,string,number,number,string,number]> = [];
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  GEONAMES_CITIES = require("./city-data-geonames").GEONAMES_CITIES ?? [];
  if (GEONAMES_CITIES.length) {
    console.log(`📍 GeoNames: ${GEONAMES_CITIES.length.toLocaleString()} cities loaded.`);
  }
} catch {
  // file doesn't exist yet — will fall back to EXTENDED_CITIES
}
...
// The built-in detailed cities already in CITIES above are always kept as-is.
{
  ...
  const source: Array<[string,string,number,number,string,number]> =
    GEONAMES_CITIES.length > 0 ? GEONAMES_CITIES : EXTENDED_CITIES;

  if (GEONAMES_CITIES.length > 0) {
    console.log(`   Using GeoNames (${GEONAMES_CITIES.length.toLocaleString()} cities).`);
  } else {
    console.log(`   GeoNames not found — using EXTENDED_CITIES (${EXTENDED_CITIES.length} cities).`);
