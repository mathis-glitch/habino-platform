...
export function PropertyCard({ property: p, compact = false }: PropertyCardProps) {
  ...
  if (compact) {
    ...
    return (
      <Link href={`/properties/${p.id}`}
        className="group flex items-center bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden">
        {/* Thumbnail */}
  ...
  // ── Standard grid card ────────────────────────────────────────────────────
  return (
    <div className="group relative bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden">
      {/* Save button — heart icon matching Figma */}
      <button
        onClick={(e) => { e.preventDefault(); toggle(p.id); }}
        className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-white/95 flex items-center justify-center shadow-sm hover:scale-110 transition-transform"
      ...
      <Link href={`/properties/${p.id}`} className="block">
        ...
        <div className="relative h-52 bg-slate-100 overflow-hidden">
          ...
          <div className="absolute top-3 left-3">
            <span className={`px-2.5 py-1 rounded-md text-xs font-semibold text-white ${badgeBg}`}>
              {badge}
            </span>
          </div>
          {/* Property type chip */}
          <div className="absolute bottom-3 left-3">
            <span className="px-2 py-0.5 rounded-md bg-black/35 backdrop-blur-sm text-white text-[10px] font-medium capitalize">
              {p.property_type}
            </span>
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          {/* Title */}
          <p className="font-semibold text-slate-800 text-sm line-clamp-1 mb-1">{p.title}</p>
          ...
          {/* Specs row — matches Figma's beds/baths/size row */}
