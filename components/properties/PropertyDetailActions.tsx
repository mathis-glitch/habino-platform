...
function useIsLoggedIn() {
  ...
  useEffect(() => {
    fetch("/api/auth/session")
      ...
      .then(d => setLoggedIn(!!d?.user?.id))
      .catch(() => setLoggedIn(false));
  }, []);
  return loggedIn;
}

// ── Save button (no auth gate — browsing feature) ─────────────────────────────
...
export function StickyContactBar({ agentPhone, agentEmail, propertyTitle }: Props) {
  ...
  function handleProtectedAction(e: React.MouseEvent, href: string) {
    if (loggedIn === false) {
      e.preventDefault();
      setAuthGate(true);
      return;
    }
    // loggedIn === true or null (still loading) → allow
  ...
  return (
    <>
      <div
        style={{
          position: "fixed", bottom: 64, zIndex: 40,
      ...
      >
        {whatsappNum && (
          <a
            ...
            onClick={(e) => handleProtectedAction(e, `https://wa.me/${whatsappNum}?text=${whatsappMsg}`)}
            target="_blank"
        ...
        {agentPhone && (
          <a
            href={`tel:${agentPhone}`}
            onClick={(e) => handleProtectedAction(e, `tel:${agentPhone}`)}
        ...
        {agentEmail && !agentPhone && (
          <a
            ...
            onClick={(e) => handleProtectedAction(e, `mailto:${agentEmail}`)}
        ...
        {!agentPhone && !agentEmail && (
          <button
            ...
            style={{
              ...
              background: "#2D6A4F", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer",
            }}
          >
            Contact agent
          </button>
      ...
      <AuthGateModal
        ...
        reason="Sign up for free to contact this agent via WhatsApp, call, or email."
      />
    </>
  );
}
