  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages: [{ role: "user", content: `Book a viewing for: ${propertyTitle}` }],
          bookingDirect: {
  ...
  if (done) {
    return (
      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 text-center">
        ...
        <p className="font-semibold text-emerald-800">Viewing request sent!</p>
        <p className="text-sm text-emerald-600 mt-1">We'll be in touch by email to confirm.</p>
      </div>
    );
  }
